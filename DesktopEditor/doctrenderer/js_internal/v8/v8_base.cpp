...
namespace NSJSBase
{
	...
	void generateSnapshot(const std::string& strScript, const std::wstring& strSnapshotFilePath)
	{
		...
		v8::SnapshotCreator snapshotCreator(isolate, externalRefs);
		{
			...
			// Compile and run
			v8::Local<v8::String> source = v8::String::NewFromUtf8(isolate, strScript.c_str()).ToLocalChecked();
			v8::Local<v8::Script> script = v8::Script::Compile(context, source).ToLocalChecked();

			if (!script->Run(context).IsEmpty())
			{
				std::cout << "Snapshot was created successfully" << std::endl;
				std::cout << "Note: snapshot will be used at next program run" << std::endl;
			}
		...
		if (data.data && snapshotFile.CreateFileW(strSnapshotFilePath))
		{
	...
	JSSmart<CJSValue> CJSContext::runScript(const std::string& script, JSSmart<CJSTryCatch> exception, const std::wstring& scriptPath)
	{
		...
		// currently scriptPath is only specified when we compile SDK JS scripts
		// so we can use it as a flag to generate snapshot instead
		if(!scriptPath.empty())
		{
#ifdef USING_SNAPSHOT
			std::wstring strSnapshotFilePath = CV8Worker::m_sExternalDirectory + L"/snapshot.bin";
			if (!NSFile::CFileBinary::Exists(strSnapshotFilePath))
			{
				generateSnapshot(script, strSnapshotFilePath);
			}
			else
			{
				std::cout << "Using snapshot..." << std::endl;
				CJSValueV8* _return = new CJSValueV8();
				_return->value = v8::Undefined(V8IsolateOneArg);
				return _return;
			}
#endif
			std::cout << "Compile and run sdk scripts without snapshot..." << std::endl;
